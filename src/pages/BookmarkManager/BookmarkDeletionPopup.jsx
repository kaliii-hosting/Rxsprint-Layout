...
import { Trash2, Lock, AlertTriangle, X, Bookmark, Folder } from 'lucide-react';
import '../../components/DeletionConfirmPopup/DeletionConfirmPopup.css';

const BookmarkDeletionPopup = ({ 
  isOpen, 
  item, 
  onConfirm, 
  onCancel 
  ...
  const Icon = item.type === 'folder' ? Folder : Bookmark;
  const itemType = item.type === 'folder' ? 'Folder' : 'Bookmark';
  ...
  return (
    <div className="deletion-popup-overlay" onClick={onCancel}>
      ...
      >
        ...
        <h2 className="deletion-title">Delete {itemType}</h2>
        ...
        <p className="deletion-warning">
          {item.type === 'folder' 
            ? 'All bookmarks inside this folder will also be deleted.'
...
export default BookmarkDeletionPopup;